    ...
    <div className="auth-page grid min-h-screen grid-cols-1 bg-[linear-gradient(180deg,#fff8f6_0%,var(--background)_100%)] md:grid-cols-2">
      <aside className="auth-brand-panel hidden min-h-screen flex-col items-center justify-center bg-[linear-gradient(135deg,#7d4038_0%,#c97970_100%)] p-10 text-white md:flex">
        <div className="max-w-[420px] text-center">
          ...
          <h1 className="mb-4 text-[42px] font-extrabold leading-tight text-white">Hospital Partner Portal</h1>
          <p className="text-lg font-semibold leading-8 text-white/90">
            Review postpartum alerts, coordinate care, and stay ready for urgent referrals.
          </p>
          <div className="mt-8 grid gap-3 text-left">
            {["Emergency-ready workspace", "Patient referral overview", "Care team access"].map((item) => (
      ...
      <main className="auth-form-panel flex min-h-screen flex-col items-center justify-center px-5 py-8 md:p-10">
        <div className="auth-card w-full max-w-[430px]">
          ...
          <div className="mb-8">
            ...
            <h2 className="mb-2 text-[34px] font-black leading-tight text-[var(--color-dark)]">Hospital Sign In</h2>
            <p className="text-base font-semibold text-[var(--color-text-muted)]">Access your MamaGuard care team workspace</p>
          </div>

          <form
            className="auth-form flex flex-col gap-[18px]"
            onSubmit={(event) => {
          ...
          >
            ...
            <FieldShell label="Password">
              ...
              <input className="min-w-0 flex-1 bg-transparent text-base text-[var(--color-dark)] outline-none" type={showPassword ? "text" : "password"} placeholder="Enter password" />
          ...
          <div className="auth-switch mt-8 border-t border-[var(--color-border)] pt-8 text-center">
            <p className="text-base text-[var(--color-text-muted)]">
              ...
              <Link href="/hospital-register" className="font-extrabold text-[var(--color-primary)] no-underline hover:opacity-75">
                Sign Up
              </Link>
            </p>
            <p className="mt-3 text-sm text-[var(--color-text-muted)]">
              Not a hospital?{" "}
              <Link href="/login" className="font-extrabold text-[var(--color-primary)] no-underline hover:opacity-75">
                Patient sign in
              </Link>
            </p>
          </div>
